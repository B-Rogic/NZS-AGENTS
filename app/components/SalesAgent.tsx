...
import NextProgressButtonAndPreview from "./NextProgressButtonAndPreview";
        ...
        <div className="h-screen overflow-y-scroll">
            <div className="bg-gradient-agent h-screen flex flex-col justify-between pt-10 gap-10 items-center pb-20 md:px-5 px-3">
                ...
                <div className="flex border border-white justify-center items-center relative rounded-full p-1">
                    <Image src={steps[0].profile ? steps[0].profile : profile} width={1000} height={1000} alt="profile" className="w-40 h-40 object-cover rounded-full" />
                </div>
        ...
        <>
            ...
            <div className="flex justify-between">
                {steps?.map((step, index) => (
                    <ProgressComponent key={index} currentIndex={currentIndex + 1} step={step.tabInfo} index={index + 1} />
                ))}
            </div>
            {currentIndex === 0 && (
                ...
                <div className="flex flex-col gap-3 w-full">
                    ...
                    {currentStep.state?.map((st, index) => (
                        <ButtonSimulation
                        className={`${stateActive === st.name ? "bg-majorgreen text-superlightgreen" : "bg-superlightgreen text-majorgreen"} md:text-md text-sm`}
                        key={index}
                        text={st.name}
                        />
                    ))}
            ...
            {currentIndex === 3 && (
                <div className="my-10 bg-superlightgreen rounded-2xl p-3">
                    ...
                    <div className="w-30 h-30 mx-auto rounded-full relative my-5">
                        <Image src={steps[0].profile ? steps[0].profile : profile} width={1000} height={1000} alt={currentStep.name || "Photo"} className="rounded-full w-full h-full object-cover" />
                    </div>
            ...
            <NextProgressButtonAndPreview previewTitle="APPLICATION DETAILS" progress={progress} handleNext={() => progress >= 100 ? setFinished(true) : nextStep()} />
        </>
        </GlobalContainer>
      )}
    </>
